...
fn compute_grade_distribution(v: &Vec<i32>) -> Vec<i32> {
    let mut distribution: Vec<i32> = Vec::with_capacity(5);
    for _i in 1..6 {
        distribution.push(0);
    }
    for x in v {
        if *x >= 0 && *x < 60 {         // F letter grade
            distribution[4] += 1;
        } else if *x >= 60 && *x < 70 { // D letter grade
            distribution[3] += 1;
        } else if *x >= 70 && *x < 80 { // C letter grade
            distribution[2] += 1;
        } else if *x >= 80 && *x < 90 { // B letter grade
            distribution[1] += 1;
        } else if *x >= 90 {            // A letter grade
            distribution[0] += 1;
        }
    }
    distribution
